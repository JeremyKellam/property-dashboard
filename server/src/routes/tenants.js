...
router.post('/', async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO tenants (unit_number, tenant_name, phone, email, lease_start, lease_end, monthly_rent, notes)
     ...
     RETURNING *`,
    [unit_number, tenant_name, phone, email, lease_start, lease_end, monthly_rent, notes]
  );
...
router.put('/:id', async (req, res) => {
  ...
  const result = await pool.query(
    ...
    [tenant_name, phone, email, lease_start, lease_end, monthly_rent, notes, req.params.id]
  );
  res.json(result.rows[0]);
});
